...
namespace Petter
{
	...
	*/
	...
	// version of bnb with an hardcoded regularizer. It discards the branches that have at least k ones
	template<typename real>
	real branch_and_bound(const PseudoBoolean<real>& f, vector<label>& x, BBInfo* bbinfo)
	{
		using namespace std;
		...
		while (!problems.empty()) {
			...
			if (nlabeled == int(n) && problem.bound < upper_bound) {
			...
			else if (nlabeled < int(n) && problem.bound <= upper_bound && no_of_ones < regularizer)  {
				// Need to branch, because this subproblem
				// is not completely solved
				should_branch = true;
				...
				// >= regularizer entries have been labeled, so label the unlabeled ones as 0 
				// then push the resulting problem to the queue
				for (size_t i=0;i<n;++i) {
					// <0 means unlabeled
					if(problem.fixed[i] < 0){
						problem.fixed[i] = 0;
